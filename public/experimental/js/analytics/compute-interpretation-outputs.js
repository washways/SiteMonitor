(function (global, factory) {
    ...
    const CATEGORY_DEFINITIONS = [
        ...
        },
        {
            ...
            suggested_action: "Review pumping conditions and schedule a field or operational check."
        },
        {
            ...
            suggested_action: "Prioritize investigation and compare against earlier operating periods."
        },
        {
    ...
    function determineStatusCategory(row, cohort = {}) {
        ...
        const stressSignals = row.stress_flag || maxDrawdown >= cohort.drawdownReviewThreshold || (Number.isFinite(medianQs) && medianQs > 0 && medianQs <= 0.5);
        const severeReliabilityConcern = downtimeShare >= 0.75 || (unreliableIndex >= cohort.unreliabilityThreshold && activeShare <= 0.25);
        ...
        if (row.performance_decline_flag && (stressSignals || activeShare >= 0.25)) {
    ...
    function determineReasons(row, statusCategory, cohort = {}) {
        ...
        if (row.performance_decline_flag) reasons.push("recent_performance_decline");
        ...
        if (Number.isFinite(medianQs) && medianQs > 0 && medianQs <= 0.5) reasons.push("low_specific_capacity");
    ...
    function computePriorityScore(row, statusCategory, cohort = {}) {
        ...
        if (statusCategory === "high_use_but_stable") score += 30;
        if (statusCategory === "stressed") score += 65;
        if (statusCategory === "declining_performance") score += 75;
        if (statusCategory === "unreliable_or_possible_fault") score += 80;
        if (statusCategory === "insufficient_data") score += 15;

        score += Math.min(10, activeShare * 12);
        score += Math.min(10, downtimeShare * 20);
        score += Math.min(15, unreliableIndex * 25);
        if (totalVolume >= cohort.highUseVolumeThreshold) score += 5;
        if (maxDrawdown >= cohort.drawdownReviewThreshold) score += 10;
        if (Number.isFinite(medianQs) && medianQs > 0 && medianQs <= 0.5) score += 10;
        if (row.performance_decline_flag) score += 5;

        return Math.max(0, Math.min(100, Math.round(score)));
    }

    function priorityLabel(score) {
        if (score >= 80) return "urgent investigation";
        if (score >= 55) return "planned review";
    ...
    function buildInterpretationText(row, statusCategory) {
        ...
        if (statusCategory === "high_use_but_stable") {
            return `This borehole is carrying a relatively high operational load (${round(volume, 1)} m³ observed) but is not currently showing the main stress triggers.`;
        }
        if (statusCategory === "stressed") {
            return `This borehole shows a stressed pattern, with recent drawdown reaching about ${round(drawdown, 2)} m${Number.isFinite(qs) ? ` and median Q/S near ${round(qs, 3)}` : ""}.`;
        }
    ...
    function computeInterpretationOutputs(boreholeRows = [], network = {}, options = {}) {
        ...
        const drawdownReviewThreshold = Number.isFinite(Number(options.drawdownReviewThreshold))
            ...
            : Math.max(2, percentile(rows.map((row) => row.max_drawdown_observed_m), 0.75) || 2);
        const unreliabilityThreshold = Number.isFinite(Number(options.unreliabilityThreshold))
            ? Number(options.unreliabilityThreshold)
            : Math.max(0.75, percentile(rows.map((row) => row.data_unreliability_index), 0.9) || 0.75);
        const cohort = { highUseVolumeThreshold, drawdownReviewThreshold, unreliabilityThreshold };
